...
public final class MatchingBusStops20 {
	...
	public static Dataset<GeoObject> generateDataFrames2(String dataSource1, String dataSource2, String sourceType, SparkSession spark) throws Exception {
		JavaSparkContext ctx = new JavaSparkContext(spark.sparkContext());
		JavaRDD<String> source1 = ctx.textFile(dataSource1);
		
		
		JavaRDD<GeoObject> x = source1.flatMap(new FlatMapFunction<String, GeoObject>() {

			@Override
			public Iterator<GeoObject> call(String t) throws Exception {
				List<GeoObject> output = new ArrayList<GeoObject>();
				if (!t.contains("geometry")) {
					String[] attributes = t.split(";");
					GeoPoint2 point = new GeoPoint2(attributes[0], attributes[1], InputTypes.GOV_POLYGON, Integer.parseInt(attributes[2]), Integer.parseInt(attributes[3]));
					output.add(point);
				}
		...
		Dataset<GeoObject> pointsDS1 = spark.createDataset(x.collect(), geoObjEncoder);
...
//		});
		...
		Accumulator<Double> count = ctx.accumulator(0.0);
