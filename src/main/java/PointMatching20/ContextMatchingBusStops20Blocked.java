 ...
 *
 */
public final class ContextMatchingBusStops20Blocked {
	...
	public static Dataset<String> run(Dataset<GeoObject> points, double thresholdLinguistic, double thresholdPointDistance, Integer amountPartition, SparkSession spark) throws Exception {
		...
		Encoder<Tuple2<String, GeoObject>> tupleEncoder = Encoders.tuple(Encoders.STRING(), geoObjEncoder);
		
		Dataset<Tuple2<String, GeoObject>> pointsPaired = points.flatMap(new FlatMapFunction<GeoObject, Tuple2<String, GeoObject>>() {

			@Override
			public Iterator<Tuple2<String, GeoObject>> call(GeoObject s) throws Exception {
				List<Tuple2<String, GeoObject>> listOfPointTuple = new ArrayList<Tuple2<String, GeoObject>>();
				
				//define key blocks for geographic context
				String keyX;
				
				if(s.getGeometry().getCentroid().getX() < 0){
					keyX = ((-1 * s.getGeometry().getCentroid().getX())+"").substring(0, 4);
				} else {
					keyX = (s.getGeometry().getCentroid().getX()+"").substring(0, 4);
				}
				
				String keyY;
				if(s.getGeometry().getCentroid().getY() < 0){
					keyY = ((-1 * s.getGeometry().getCentroid().getY())+"").substring(0, 4);
				} else {
					keyY = (s.getGeometry().getCentroid().getY()+"").substring(0, 4);
				}
				
				String key = keyX + keyY;
				
				listOfPointTuple.add(new Tuple2<String, GeoObject>(key, s));
...
//				}
		...
		KeyValueGroupedDataset<String, Tuple2<String, GeoObject>> pointsGrouped = pointsPaired.groupByKey(new MapFunction<Tuple2<String, GeoObject>, String>() {

			@Override
			public String call(Tuple2<String, GeoObject> value) throws Exception {
				return value._1();
			}
		}, Encoders.STRING());
		...
		Dataset<Tuple2<Integer, PointPair>> matches = pointsGrouped.flatMapGroups(new FlatMapGroupsFunction<String, Tuple2<String, GeoObject>, Tuple2<Integer, PointPair>>() {
			@Override
			public Iterator<Tuple2<Integer, PointPair>> call(String key, Iterator<Tuple2<String, GeoObject>> values)
					throws Exception {
