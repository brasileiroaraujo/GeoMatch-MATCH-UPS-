...
public final class MatchingBusStops20Blocked {
	...
	public static Dataset<String> run(Dataset<GeoObject> points, double thresholdLinguistic, double thresholdPointDistance, Integer amountPartition, SparkSession spark) throws Exception {
		...
		Dataset<Tuple2<String, GeoObject>> pointsPaired = points.flatMap(new FlatMapFunction<GeoObject, Tuple2<String, GeoObject>>() {
			...
			public Iterator<Tuple2<String, GeoObject>> call(GeoObject s) throws Exception {
				...
				if(s.getGeometry().getCoordinate().x < 0){
					keyX = ((-1 * s.getGeometry().getCoordinate().x)+"").substring(0, 4);
				} else {
					keyX = (s.getGeometry().getCoordinate().x+"").substring(0, 4);
				}
				
				String keyY;
				if(s.getGeometry().getCoordinate().y < 0){
					keyY = ((-1 * s.getGeometry().getCoordinate().y)+"").substring(0, 4);
				} else {
					keyY = (s.getGeometry().getCoordinate().y+"").substring(0, 4);
				}
