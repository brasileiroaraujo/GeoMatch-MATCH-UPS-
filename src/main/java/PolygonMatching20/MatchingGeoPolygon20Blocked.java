...
public final class MatchingGeoPolygon20Blocked {
...
//		Encoder<GeoPolygon> polygonEncoder = Encoders.bean(GeoPolygon.class);
	...
	public static Dataset<String> run(Dataset<GeoPolygon> polygons, double thresholdLinguistic, double thresholdPolygon, Integer amountPartition, SparkSession spark) throws Exception {
		...
		Encoder<Tuple2<String, GeoPolygon>> tupleEncoder = Encoders.tuple(Encoders.STRING(), polygonEncoder);
		
		Dataset<Tuple2<String, GeoPolygon>> polygonLabed = polygons.flatMap(new FlatMapFunction<GeoPolygon, Tuple2<String, GeoPolygon>>() {
			...
			public Iterator<Tuple2<String, GeoPolygon>> call(GeoPolygon s) throws Exception {
				...
				//define key blocks for geographic context
				String keyX;
				if(s.getCentroid().getX() < 0){
					keyX = ((-1 * s.getCentroid().getX())+"").substring(0, 3);
				} else {
					keyX = (s.getCentroid().getX()+"").substring(0, 3);
				}
				
				String keyY;
				if(s.getCentroid().getY() < 0){
					keyY = ((-1 * s.getCentroid().getY())+"").substring(0, 3);
				} else {
					keyY = (s.getCentroid().getY()+"").substring(0, 3);
				}
				
				String key = keyX + keyY;
				
				listOfPolygonTuple.add(new Tuple2<String, GeoPolygon>(key, s));
				
				
				//define key blocks for linguistic context
				GeoPolygon sLing = new GeoPolygon(s);
				sLing.setGeoBlockLabel(key);
				if (sLing.getGeoName().length() >= 3) {
					listOfPolygonTuple.add(new Tuple2<String, GeoPolygon>(sLing.getGeoName().substring(0, 3), sLing));
				} else {
					listOfPolygonTuple.add(new Tuple2<String, GeoPolygon>(sLing.getGeoName(), sLing));
				}
				
				return listOfPolygonTuple.iterator();
				
//				if (s.getType().equals(InputTypes.OSM_POLYGON)) {
//					listOfPolygonTuple.add(new Tuple2<String, GeoPolygon>(, s));
//					return listOfPolygonTuple.iterator();
//				} else { //equals to InputTypes.GOV_POLYGON
//					for (int i = 0; i < numReplication.value(); i++) {
//						listOfPolygonTuple.add(new Tuple2<String, GeoPolygon>(i, s));
//					}
//					return listOfPolygonTuple.iterator();
//				}
			}
		...
		KeyValueGroupedDataset<String, Tuple2<String, GeoPolygon>> polygonsGrouped = polygonLabed.groupByKey(new MapFunction<Tuple2<String, GeoPolygon>, String>() {

			@Override
			public String call(Tuple2<String, GeoPolygon> value) throws Exception {
				return value._1();
			}
		}, Encoders.STRING());
		...
		Dataset<Tuple2<Integer, PolygonPair>> matches = polygonsGrouped.flatMapGroups(new FlatMapGroupsFunction<String, Tuple2<String, GeoPolygon>, Tuple2<Integer, PolygonPair>>() {
			@Override
			public Iterator<Tuple2<Integer, PolygonPair>> call(String key, Iterator<Tuple2<String, GeoPolygon>> values)
					throws Exception {
				...
				for (GeoPolygon entSource : polygonsSource) {
					...
					for (GeoPolygon entTarget : polygonsTarget) {
						//do not match twice
						if ((entSource.getGeoBlockLabel() == null && entTarget.getGeoBlockLabel() == null) || (!entSource.getGeoBlockLabel().equals(entTarget.getGeoBlockLabel()))) {
							...
							if (linguisticSimilarity > thresholdLinguistic && polygonSimilarity > thresholdPolygon) {
								...
								//This condition avoid that a polygon which contains a match consider other polygons just because the name similarity
								if (!findMatch && polygonSimilarity != 0.0) {
